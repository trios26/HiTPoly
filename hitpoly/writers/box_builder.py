...
def create_ligpargen(
    ...
    if platform == "local":
        ...
        command = "LigParGen -m poly.mol -o 0 -c 0 -r PLY -d . -l"
...
def get_charges_htvs(smiles, htvs_path=None, htvs_details: dict = None):
    import django
    ...
    from pgmols.models import Species, Geom, Group, Calc

    geoms = Geom.objects.filter(
        species__group__name="lipoly",
        parentjob__config__name=htvs_details["geom_config_name"],
    ).all()
    geoms = geoms.filter(species__smiles=smiles, childjobs__status="done")
    geom_ids = [i[0] for i in list(geoms.values_list("id"))]

    if "calc_method_name" not in htvs_details:
        htvs_details["calc_method_name"] = "dft_wb97xd3_def2tzvp"

    charges = Calc.objects.filter(
        method__name=htvs_details["calc_method_name"],
        parentjob__group__name="lipoly",
        geoms__id__in=geom_ids,
        props__atomiccharges__resp__isnull=False,
    )
    charges_values = charges.values_list("props__atomiccharges__resp")
    char = np.array(charges_values).reshape(-1, np.array(charges_values).shape[-1])
    atoms = [NUM_TO_ELEMENT[int(i[0])] for i in geoms[0].xyz]
    return char.mean(axis=0), char.std(axis=1), atoms
...
def assign_lpg_params(
    ...
    # The paths for these charges probably have to be updated eventually
    if charges == "DFT":
        print("Loading DFT charges")

        charges_df = pd.read_csv(f"{lit_charges_save_path}/DFT_CHARGES/{poly_name}.csv")
        assert len(charges_df) == len(atoms_short)

        charges_dict = {}
        for i in charges_df.groupby("names", as_index=False).mean().to_dict("records"):
...
def creating_ff_and_resid_files_gromacs(
    ...
    # ff_file.append("[ atoms ]")
    # # I've no idea what this cgnr is and what does it mean
    # ff_file.append(";   nr       type  resnr residue  atom   cgnr     charge       mass")
    # atom_ind = 0
    # cgnr = 1
    # atom_ind_name_dict = {}
    #     for ind, (atom_pdb, param) in enumerate(
    ...
    #         if atom_ind+1 == 32*cgnr:
    #             cgnr += 1
    #         string = f'{atom_ind+1:>6}{a_name:>11}{resid_ind:>6}{n:>7}'
    #         string1 = f'{a_name:>6}{str(cgnr):>7}{str(round(param[0],3)):>11}{str(round(a_mass,3)):>11}'
    #         ff_file.append(string+string1)
    #         atom_ind_name_dict[a_name] = atom_ind+1
    #         atom_ind += 1
...
def get_concentration_from_molality(
    ...
    def change_chain_length(long_smiles, repeat_units):
        if get_atom_count(long_smiles) > polymer_chain_length + int(
        ...
        elif get_atom_count(long_smiles) < polymer_chain_length - int(
            polymer_chain_length / 10
        ):
            repeat_units += 1
            long_smiles, repeats = create_long_smiles(
    ...
    while get_atom_count(long_smiles) > polymer_chain_length + int(
        polymer_chain_length / 10
    ) or get_atom_count(long_smiles) < polymer_chain_length - int(
        polymer_chain_length / 10
    ):
